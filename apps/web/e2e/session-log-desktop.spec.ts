...
test.describe("@desktop session log", () => {
  ...
  test("E: pre-session sleep chip persists to the wellness row (Phase 3 B1)", async ({
    page,
    context,
    freshUser,
    seedConfig,
    ...
    // Required DC-P1 chips first so the submit button enables.
    await page
      .getByRole("radiogroup", { name: /fatigue level/i })
      .getByRole("radio", { name: /good/i })
      .click();
    ...
    // Phase 3 B1 — tap the "8h+" sleep chip.
    await page.getByTestId("sleep-chip-gte8").click();

    await page.getByRole("button", { name: /^⚡ start /i }).click();
    await page.waitForURL(/\/app\/sessions\/[0-9a-f-]{36}(?:\?|$|#)/, { timeout: 15_000 });

    // Service-role: today's wellness row should now carry sleep_hours=8.5.
    ...
    expect(Number(wellness!.sleep_hours)).toBeCloseTo(8.5, 1);
  });
});
