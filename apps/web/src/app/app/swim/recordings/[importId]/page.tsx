...
import { loadRecordingSwimClaims } from "@/lib/swim/recording-claims";
...
export default async function SwimRecordingPage({ params, searchParams }: {
  ...
  const requestedWorkout = z.string().uuid().safeParse(query?.workout);
  const origin = parseSwimOrigin(query?.from);
  const context = requestedWorkout.success
    ? `?workout=${requestedWorkout.data}${origin ? `&from=${origin}` : ""}`
    : origin ? `?from=${origin}` : "";
  const back = origin === "sessions" ? swimReturnDestination(origin)
    : requestedWorkout.success
      ? { href: `/app/swim/${requestedWorkout.data}${origin ? `?from=${origin}` : ""}`, label: "Workout" }
      : origin ? swimReturnDestination(origin) : { href: "/app/settings/swimming", label: "Swimming imports" };
  const evidence = recording.evidence;
  const matched = current?.metadata.workout;
  const latest = latestId === recording.id;
  const outcomesAvailable = await swimImportOutcomesAvailable(client);
  const claims = outcomesAvailable ? await loadRecordingSwimClaims(client, user.id, recording.id) : [];
  let currentOutcome = null;
  if (outcomesAvailable && current?.workout_id && current.import_id === recording.id && matched) {
    ...
    const enabled = latest && evidence.environment === "pool" && current.import_id === recording.id &&
      state.canConfirm && parsed.data.status === "scheduled" && parsed.data.session_id === null &&
      process.env.SWIM_IMPORT_OUTCOMES_ENABLED === "true";
    if (enabled || (confirmation?.metadata.outcome != null && claims.some((claim) => claim.id === current.workout_id))) {
      currentOutcome = <RecordingOutcome key={`${current.id}:${confirmation?.id ?? "none"}:${parsed.data.revision}`}
        workoutId={current.workout_id} workoutTitle={matched.title} origin={origin}
        matchId={current.id} workoutRevision={parsed.data.revision}
  ...
  return <main className={styles.page}>
    ...
    />}
    {currentOutcome}
    {claims.filter((claim) => current?.import_id !== recording.id || claim.id !== current.workout_id).map((claim) => <RecordingOutcome
      key={`${claim.id}:${claim.outcomeId}:${claim.workoutRevision}`}
      workoutId={claim.id} workoutTitle={claim.title} origin={origin} matchId={null} workoutRevision={claim.workoutRevision}
      expectedOutcomeId={claim.outcomeId} current={claim.status === "needs_review" ? null : claim.claimedOutcome}
      enabled={false} canRemove needsReview={claim.status === "needs_review"} otherMatch={false}
    />)}
  </main>;
}
