...
export async function MatchedRecordings({ client, userId, workoutId, revision }: {
  client: SupabaseClient; userId: string; workoutId: string; revision: number;
}) {
  ...
  return <section className={styles.section}>
    ...
    <ul className={styles.list}>{matches.map((match) => <li key={match.id}>
      <Link className={styles.row} href={`/app/swim/recordings/${match.import_id}`}>
