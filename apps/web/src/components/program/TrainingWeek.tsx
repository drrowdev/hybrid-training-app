...
export function TrainingWeek({ entries, today, heading = "This week", authoredBlockId, authoredBlockIds = [], programLabels = {}, primaryPreviewIds = [], swimStatuses = {}, sessionLinks = {}, templateBlockIds = [], showWorkoutEditing = true }: {
  ...
  return <section className={styles.panel} aria-label={heading}>
    <div className={styles.row}><h2>{heading}</h2><Link className={styles.button} href="/app/plan">Schedule</Link></div>
        ...
        <div className={styles.week}>{workouts.length === 0 ? <span className={styles.muted}>{rest ? "Planned rest" : "No workout scheduled"}</span>
          : workouts.map((entry) => {
            ...
            const href = preview ? `#session=${entry.id}` : entry.source === "swim" ? swimWorkoutHref(entry.id, "today")
              : entry.source === "session" ? `/app/sessions/${entry.id}`
              : sessionLinks[entry.id] ? `/app/sessions/${sessionLinks[entry.id]}` : `/app/sessions/start/${entry.id}`;
            ...
            return <div key={`${entry.source}:${entry.id}`}><WorkoutLink className={styles.weekWorkout}
              style={{ textDecoration: "none", color: "var(--cp-text)", padding: "8px 0" }} href={href}>
              <div><strong>{programWorkoutTitle(entry.title, entry.programId !== null && templateBlockIds.includes(entry.programId))}</strong>{programLabel && <div className={styles.muted}>{programLabel}</div>}</div><span className={styles.muted}>{swimStatus && swimStatus !== "scheduled" && swimStatus !== "started" ? SWIM_TRAINING_LABEL[swimStatus] : swimStatus === "started" || entry.state === "started" ? "Resume" : entry.state === "completed" ? "Completed" : entry.state === "paused" ? "Paused" : preview ? "Preview" : "Start workout"}</span>
            </WorkoutLink>{showWorkoutEditing && entry.source === "primary" && entry.programId !== null && editableBlocks.has(entry.programId) && entry.state === "scheduled" && entry.date >= today &&
