...
/**
 * AiNotesEditor — inline textarea for the user's training notes.
 *
...
export type AiNotesEditorProps = {
  initialValue: string;
...
export function AiNotesEditor({
  ...
  return (
    <div
      data-testid="ai-notes-editor"
      style={{ display: "grid", gap: 6 }}
    >
      {lastUpdatedLabel && (
        <div
          data-testid="ai-notes-updated"
      ...
      >
        {pending && <span data-testid="ai-notes-status">Saving…</span>}
        {!pending && dirty && (
          <span data-testid="ai-notes-status">Unsaved · blur to save</span>
        )}
        {!pending && !dirty && savedAt != null && (
          <span data-testid="ai-notes-status">Saved</span>
        )}
        {error && (
          <span
            role="alert"
            data-testid="ai-notes-error"
