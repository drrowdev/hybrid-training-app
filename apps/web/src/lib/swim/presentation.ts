import { formatPoolCourse, formatSwimDistance, type SwimWorkout, type SwimStroke, type SwimEquipment } from "@hta/domain";
...
export function workoutPresentation(workout: SwimWorkout): Pick<SwimWorkoutView, "title" | "course" | "total" | "budgetMinutes" | "stroke" | "strokes" | "steps" | "equipment" | "pool" | "calibrationLabel"> {
  const steps: SwimWorkoutView["steps"] = [];
  ...
  workout.sections.forEach((section, sectionIndex) => {
    for (let round = 0; round < section.rounds; round += 1) {
      section.items.forEach((item, itemIndex) => {
        for (let repeat = 0; repeat < item.repeats; repeat += 1) {
          steps.push({
            id: `${sectionIndex}:${round}:${itemIndex}:${repeat}`,
            section: `${section.label}${section.rounds > 1 ? ` · Round ${round + 1}/${section.rounds}` : ""}`,
            title: `${formatSwimDistance(item.lengths, workout.snapshot.course)}${item.repeats > 1 ? ` · ${repeat + 1}/${item.repeats}` : ""}`,
            detail: [
