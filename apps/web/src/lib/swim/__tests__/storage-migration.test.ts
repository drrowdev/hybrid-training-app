...
const sql = readFileSync(fileURLToPath(new URL(
  "../../../../../../packages/db/drizzle/0146_standalone_pool_swimming.sql",
  import.meta.url,
)), "utf8");
const rollback = () => readFileSync(fileURLToPath(new URL(
  "../../../../../../packages/db/rollbacks/0146_standalone_pool_swimming.down.sql",
  import.meta.url,
)), "utf8");
...
const identityMigration = readFileSync(new URL(
  "../../../../../../packages/db/drizzle/0147_swim_request_identity.sql", import.meta.url,
), "utf8");
const identityRollback = readFileSync(new URL(
  "../../../../../../packages/db/rollbacks/0147_swim_request_identity.down.sql", import.meta.url,
), "utf8");
...
describe("ADR0079 private swimming identity migration (DC-SW8/DC-SW9)", () => {
  ...
  it("registers the additive migration without statement breakpoints", () => {
    ...
    expect(journal.entries[150]).toEqual({
      idx: 150, version: "7", when: 1789214400000,
      tag: "0150_swim_import_storage", breakpoints: false,
    });
    expect(journal.entries[151]).toEqual({
      idx: 151, version: "7", when: 1789228800000, tag: "0151_swim_pool_changes", breakpoints: false,
    });
    expect(journal.entries[152]).toEqual({
      idx: 152, version: "7", when: 1789236000000, tag: "0152_swim_private_courses", breakpoints: false,
    });
    expect(journal.entries[153]).toEqual({
      idx: 153, version: "7", when: 1789246800000, tag: "0153_swim_import_matching", breakpoints: false,
    });
    expect(journal.entries[154]).toEqual({
      idx: 154, version: "7", when: 1789304400000, tag: "0154_swim_untimed_courses", breakpoints: false,
    });
    ...
    expect(journal.entries[157]).toEqual({
      idx: 157, version: "7", when: 1789459200000, tag: "0157_swim_conditioning_lifecycle", breakpoints: false,
    });
    expect(journal.entries[158]).toEqual({
      idx: 158, version: "7", when: 1789488000000, tag: "0158_conditioning_request_identity", breakpoints: false,
    });
    expect(readdirSync(directory).filter((name) => name.endsWith(".sql"))).toHaveLength(159);
  });
});
