...
import {
  ...
  runMovementReferenceRoundTrip, snapshotSql,
...
describe("DC-SW8 composed relationship round trip lifecycle (no database execution)", () => {
  ...
  it("keeps C3 integrity failures strict and authenticated without changing the twelve identities", () => {
    ...
    expect(negative.match(/linked\.client\.from/g)).toHaveLength(7);
    ...
    // This is a source blocker, not permission to change RLS or accept a zero-row update.
