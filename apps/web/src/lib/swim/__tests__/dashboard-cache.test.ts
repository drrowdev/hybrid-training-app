...
describe("DC-SW1/DC-SW4 cache-only swimming adapter", () => {
  it("runs the synthetic SQLite, privacy, failure and read-only contract checks", () => {
    const result = runPython(["-m", "unittest", "discover", "-s", tests, "-p", "test_swim_dashboard_cache.py"]);
