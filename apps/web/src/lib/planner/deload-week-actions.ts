...
export async function previewDeloadWeekAction(
  percent?: number, boundaryKey?: string, recommendationId?: string,
): Promise<ReviewedDeloadWeekPreview | null> {
  const options = optionsSchema.parse({ percent, boundaryKey, recommendationId });
...
export async function insertDeloadWeekAction(
  percent?: number, boundaryKey?: string, recommendationId?: string, review?: DeloadScheduleReview,
...
async function prepareRemoval(client: Client, snapshot: ScheduleSnapshot, weekIndex: number): Promise<RemoveDeloadPreview | null> {
  const block = await client.from("training_blocks").select("id").eq("status", "active").is("deleted_at", null).maybeSingle();
...
export async function previewRemoveDeloadWeekAction(input: z.infer<typeof removeSchema>): Promise<RemoveDeloadPreview | null> {
  const { weekIndex } = removeSchema.parse(input);
  const client = await createClient();
  const { data: { user } } = await getAuthUser();
  if (!user) redirect("/login");
  return prepareRemoval(client, await loadTrainingSchedule(client), weekIndex);
}
...
export async function removeDeloadWeekAction(
  ...
  try {
    ...
    const preview = await prepareRemoval(client, await loadTrainingSchedule(client), parsed.weekIndex);
