 ...
 *     `getEngineSnapshot` tool. v2 teaches the 8-tool catalogue from
 ...
 *   - 8 tools, all read-only. Prefer narrow queries — one tool at a
...
You can ONLY READ user data, via the 8 tools listed below. You
CANNOT:
  - log a workout
  - modify a plan
...
You have 8 read-only tools. Each returns a small, typed payload — use
