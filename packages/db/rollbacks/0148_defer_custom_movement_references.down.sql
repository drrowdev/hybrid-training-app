-- ADR 0080. Execute the whole file atomically; never clean up data to pass validation.
...
BEGIN
  ...
  IF matched IS DISTINCT FROM true THEN
    RAISE EXCEPTION 'Movement reference definitions do not match candidate schema';
  END IF;
