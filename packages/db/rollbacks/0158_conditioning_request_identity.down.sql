-- Disable integrated conditioning before an explicitly approved rollback.
-- Restores function bodies/grant only; never deletes training history or ledger rows.
BEGIN;
...
BEGIN
  IF current_user <> 'postgres' OR NOT has_function_privilege(
    'conditioning_writer', 'public.swim_request_user_id()', 'EXECUTE'
  ) THEN RAISE EXCEPTION 'CONDITIONING_IDENTITY_BASELINE'; END IF;
  ...
  LOOP
    ...
    IF encode(sha256(convert_to(before_proc.prosrc, 'UTF8')), 'hex') <> target.repaired_hash THEN
      RAISE EXCEPTION 'CONDITIONING_IDENTITY_SOURCE_CHANGED';
    END IF;
    EXECUTE replace(pg_get_functiondef(before_proc.oid), 'public.swim_request_user_id()', 'auth.uid()');
    SELECT * INTO STRICT after_proc FROM pg_catalog.pg_proc WHERE oid = before_proc.oid;
    IF encode(sha256(convert_to(after_proc.prosrc, 'UTF8')), 'hex') <> target.original_hash
    ...
    END IF;
  END LOOP;
  REVOKE EXECUTE ON FUNCTION public.swim_request_user_id() FROM conditioning_writer;
END $rollback$;
COMMIT;
