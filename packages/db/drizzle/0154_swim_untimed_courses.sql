-- Accept an explicit absent time budget only for imported courses. No row rewrites.
...
BEGIN
  ...
  IF p_workout->'budget'->'minutes' IS NOT DISTINCT FROM 'null'::jsonb THEN
    IF p_workout->'snapshot'->'versions'->>'generator' IS DISTINCT FROM 'swim-course-1' THEN
      RAISE EXCEPTION 'Generated swimming prescriptions require a time budget.';
    END IF;
  ELSE
...
BEGIN
  ...
  IF v_setup->'sessionBudgetMinutes' IS NOT DISTINCT FROM 'null'::jsonb THEN
    IF p_definition->>'generatorVersion' IS DISTINCT FROM 'swim-course-1'
       OR p_definition->'privateCourse'->>'version' IS DISTINCT FROM 'swim-course-1' THEN
      RAISE EXCEPTION 'Generated swimming plans require a time budget.';
    END IF;
  ELSE
...
CREATE FUNCTION public.swim_untimed_course_ready()
RETURNS boolean LANGUAGE sql STABLE SECURITY INVOKER
SET search_path = pg_catalog, public
AS $$ SELECT true $$;
REVOKE ALL ON FUNCTION public.swim_untimed_course_ready() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.swim_untimed_course_ready() TO authenticated;
