...
| [`0081-swimming-import-connection.md`](../adr/0081-swimming-import-connection.md) | **Development-only storage approved (2026-09-12).** Narrow revocable pairing keys, account-owned immutable swimming observations, replay/correction protection and empty-only down. Additive migration 0150; isolated database acceptance pending. No existing review/production migration, live data transfer or inferred workout completion. |
...
| [`0072-warmup-ladder-wins-over-program-ramp.md`](../adr/0072-warmup-ladder-wins-over-program-ramp.md) | **A configured warm-up ladder wins over a program's ramp (2026-08-18).** `profiles.warmup_scheme` governed only native blocks; every program engine hardcoded a ramp, so "Skip warmups" still produced three sets. Programs now supply a DEFAULT: NULL ("never chose") keeps the program's own ramp, any stored ladder wins everywhere. Seam is the new optional `PlatformContext.warmupRamp`. DC-K4 satisfied via a derived override warning, a reversible "Follow the program" preset writing SQL NULL, and a `custom` override-event row. No migration. |
| [`0071-user-authored-session-links.md`](../adr/0071-user-authored-session-links.md) | **User-authored session links (2026-08-16).** Supersedes ADR 0026. The inferred antagonist-pairing toggle is replaced by explicit per-slot links of any size, expressed as the existing `circuit` primitive, storable outside the TB customization blob so they reach canonical templates and Activation. Circuit participation becomes per-set; the duration governor now requests solo pricing explicitly. Migration 0129 drops both `superset_accessories` columns. |
