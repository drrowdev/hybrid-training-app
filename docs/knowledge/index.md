...
| [`docs/design/brand-identity.md`](../design/brand-identity.md) | **Brand identity — S×C (updated 2026-06-16).** Consumer brand `S×C`, domain `getsxc.app`. Wordmark Archivo Bold 700 with sage `×` multiplier glyph; no "Strength × Cardio" descriptor (dropped). Dark-only sage theme: bg `#0f1310`, accent `#8fb39b` mapped to `--cp-bg`/`--cp-accent`. Header renders the wordmark as theme-adaptive live text; near-black app-icon master (never sage-tile, contrast rule); outlined-path SVGs + baked icons/splash/OG (regenerate via `scripts/generate-icons.mjs` + `generate-splash.mjs`). Usage rules + asset catalog inline. |
