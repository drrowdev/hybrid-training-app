...
- **DC-K1 — Recovered-week qualification is explicit (v2 §3.10)** [DEF] — A week is "recovered" iff **all** of the following pass: (1) every planned session in the week was logged — no skipped rows, no missed-past-due rows; (2) no logged session had sRPE > 9 (overreach signal per DC-A2); (3) average pre-session fatigue across the week's logged sessions is < 4 on the 1–5 scale (DC-P1); (4) average pre-session soreness across logged sessions is < 4; (5) at least one session was logged in the week (informativeness guard — a no-data week is "no data", not "recovered"). NULL signals (the user didn't fill the slider) PASS for that check — the cold-start ladder in DC-C13 carries the safety net when data is sparse, not a fail-closed gate inside DC-K1. The canonical helper is `isRecoveredWeek(week)` in `packages/engine/src/recovered-weeks.ts`; the per-user weekly rollup is `getWeeklyRecoveryRollup(supabase, userId)` in `apps/web/src/lib/engine/recovered-weeks.ts`. Both ship with PR `feat/smarter-ceiling-dc-k1`. *Test:* querying recovered-weeks for a user always returns weeks with the boolean flag set; all five failure paths + the NULL-pass policy are pinned by `packages/engine/src/recovered-weeks.test.ts`. **Confidence: MODERATE-LOW.**
