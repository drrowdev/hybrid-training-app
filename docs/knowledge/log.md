...
The recovery lives in `packages/domain/src/legacy-system-load-warmup.ts` and runs in memory on read. Nothing is written back and no marker is erased on disk, so provenance survives and the repair can be revised. It splits into an exact half and a lossy half, and refuses the lossy half rather than guessing. Any rung stored above zero is recovered exactly - the writer subtracted bodyweight, so adding it back is an inversion, not an estimate. A rung clamped to zero has lost its value, and is only rebuilt by REPLAYING the original writer: take the movement's own following main set for a percentage anchor and a training max, take the configured warm-up ladder, and re-run the exact arithmetic that produced the stored block, collapse rule included. If the replay reproduces the stored slots exactly, slot for slot, then it also tells us which rung of the ladder each surviving slot came from, and the whole block is restated off the ladder from there. If the replay produces anything else, the lossy half is abandoned and the exact recoveries stand alone. A clamped rung that nothing can place is left unprescribed; it is never filled with a heavier guess.
...
An earlier design detected the condition per movement and regenerated the whole ramp from today's configured scheme. Independent review rejected it on seven counts, of which two were decisive: today's scheme is not necessarily the scheme that wrote the plan, so regeneration would silently REPLAN a session the lifter had already partly performed; and collapse means slot count alone cannot tell you which rungs you are holding. The version that shipped never replans - it restates, and only after a byte-exact replay has proved it is looking at the right plan. The replay doubles as the drift guard: a lifter whose bodyweight has changed since the plan was written no longer reproduces the stored block, so the lossy half quietly declines instead of restating a ramp off the wrong number. Set counts, rep counts, ordering and array length are untouched, so `prescription_item_index` keeps addressing the same slot and already-logged sets stay attached; the one note that is cleared is the stale "bodyweight" cue on a rung that now carries load.
