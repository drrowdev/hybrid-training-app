...
**ADR 0017 (2026-05-31): ranked cardio-modality preference** — a **selection-only** layer (no load math, no CP-2 constant) that lets a user pick which cardio forms the planner programs by default, in priority order, instead of always defaulting to running. Storage: `profiles.preferred_cardio_modalities text[]` (nullable, no default → byte-identical for existing rows; CHECK ≤8 entries from the 9-modality vocabulary running/cycling/rowing/swimming/rucking/sled/elliptical/stair/ski_erg; migration `0081`, non-journaled per the 0078–0080 pattern). New pure resolver `apps/web/src/lib/planner/preferred-cardio-modality.ts` (`normalizeCardioModality`, `sanitizePreferredModalities`, `resolvePreferredCardioModality`) and DB→catalog adapter `apps/web/src/lib/planner/cardio-catalog.ts` (`loadCardioCatalog`, classifying every `pattern='cardio'` movement via the existing single-source `classifyCardioKind` + `normalizeCardioModality`). Wired in `actions.ts` at **both** block-creation cardio branches (archetype + custom): after `resolveCardioSlugForTier`, the resolver walks the ranked preference and substitutes the first modality with a **feasible same-`cardioKind`** movement (owned `equipment.cardio` + experience tier), holding intensity constant; if none qualify it keeps the archetype default (running). **Load-neutral by construction** — a prescribed cardio session's stress comes from `cardioKind` + duration + HR cap, and the modality-weighted `computeConcurrentScalar` (§11) reads *logged* `cardio_logs`, not the prescription — so a constant-kind modality swap changes no engine math (this also newly *consumes* `equipment.cardio`, previously stored-but-unused). **Empty preference = identity** (no catalog query, default running slug → golden master green). Determinism: lowest slug among equally-ranked feasible candidates; top-feasible-modality-equals-default keeps the exact curated slug. Coverage is honest: running/cycling/rowing have the full z2/threshold/vo2/alactic ladder, swimming/rucking/sled/elliptical/stair are z2-only, ski_erg has no zoned entries — so a swim-preferring user gets swims on easy days and running on interval days, stated in the UI. Write surface: server action `updatePreferredCardioModalities` (user-scoped client, ownership filter, Zod `.strict()`, server sanitize, empty→NULL) + `CardioModalitySettings` ranked editor on `/app/settings/training` and the onboarding Equipment step. Evidence: ADR 0008 / Wilson 2012 (modality is a *recovery-cost* input read from logs, hence selection is load-neutral) + endurance specificity principle (program the modality the athlete trains). New unit suites for resolver + catalog adapter; wizard routing test updated for the new prop. Test count 2815 → 2835 (+20).
...
**Offer/accept:** `lib/limitations/offer.ts:getLimitationResponseOffer` (read-only) surfaces the banner on the active-block plan page. The banner (`LimitationResponseCard`) is a per-item review: every proposed swap/drop is a checkbox (all checked by default), grouped by session, and the user can uncheck any change they want to keep. `lib/limitations/actions.ts:applyLimitationResponseSelection` re-derives the full plan server-side (never trusting the client) and `buildSelectedUpdates` intersects it with the checked keys (`sessionId#itemIndex`, see `item-key.ts`) — unknown/stale keys are ignored, so the client can only ever apply a SUBSET of what the engine independently deemed safe. Persisted via `applyPrescriptionUpdates`. Swaps preserve sets × reps and all effort cues — only the movement identity changes. (The all-apply `applyLimitationResponse` action is retained but no longer wired to the UI.)
...
**Generator biasing (ADR 0052) — `endurance_bias` ships, `strength_bias` deferred.** A Season block's emphasis now threads an optional `seasonBias` through `setupHybrid` into the generator (mirrors ADR 0016's `effortPreference`; absent ⇒ byte-identical). `endurance_bias` raises the concurrent block's easy-Z2 cardio volume via a new `endurance_biased` creep tier (`{ creepPerWeek: 0.07, cap: 0.15, includeShortEasy: true }`) in `cardioProgressionPlan` — strictly BEFORE the ceiling (CP-4 untouched), strength anchors held, derived from `season_blocks.emphasis` at activation in `createProgramInstance`. `strength_bias` is a generator **no-op for now** (it would need to *remove* cardio — higher detraining risk) and is deferred pending Season validation data, expressing itself only through program/template SELECTION + the advisory floor. `seasonBias=null` (every non-Season Hybrid deploy) stays golden-master byte-identical.
