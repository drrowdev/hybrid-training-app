...
* **Zone weights** (`cardio-intensity.ts:47-53`): Z1 `0.5`, Z2 `0.8`, Z3 `1.2`, Z4 `1.8`, Z5 `2.2`.
* **Fallback** (no HR zones): `clamp(rpe / 10, 0.3, 1.0)`; if RPE is null, returns `0.5`.

Shipped by **PR #167** (`feat(engine): HR-aware cardio bucket + region load when zones available`). Before that, the cardio side was duration-only — a 60 min Z2 ride and a 60 min Z5 race produced the same bucket load. Now the multiplier is per-zone, but the implementation picks **one zone per session** by avg HR (see §13 / §16). Time-in-zone is not multiplied.
...
* "Real miss" = AMRAP top set reps below target OR weight below `prescribed × 0.95`.
...
A session's avg HR is bucketed into a **single** zone for the cardio-intensity multiplier (§5). Time-in-zone is not used. This systematically over-credits time outside the dominant zone — a session that averages mid-Z2 but spent 30 % in Z3 reads as pure Z2.
...
* **HR zone time-in-zone is not used.** §5 / §16 — a session is bucketed to one zone by avg HR. Spec wants TIZ weighting.
